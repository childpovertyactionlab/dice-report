...
### Learn with Train ### 
lwt <- impact %>%
  filter(partner == "Learn With Train ") 
lwt_events <- nrow(lwt)
lwt_people_served <- sum(lwt$people_served, na.rm = TRUE)
...
### Robin Hood INC ### 
robin_hood_inc <- 
  impact %>% filter(partner %in% c("Robbin Hood INC", "Robbin Hood Inc", "Robbin Hood ", "Robbin Hood. Inc ", "Robbin Hood Inc.", "Robbin Hood Inc ", "Robbin Hood INC and Oak Cliff Veggie Project")) 
...
### Not My Son ### 
not_my_son <- impact %>%
  filter(partner %in% c("Not My Son", "Not My Son Limited ", "Tramonica Brown ")) 
...
goldmark_cultural <- impact %>%
  filter(partner %in% c("Goldmark Cultural Center", "Goldmark Cultural Center "))
...
pop_up_comfortt <- impact %>%
  filter(partner %in% c("Pop Up Comfortt", "Pop Up Comfort", "Pop Up Comfort ")) 
...
### Serun Foundation ###
serun <- impact %>%
  filter(partner %in% c("Serun Foundation", "Serun", "The SERUN Foundation")) 
...
# Filter to just Bachman Lake Together
bachman_lt <- impact %>%
...
::: g-col-12
```{=html}
<p style="text-align: center;font-weight: bold; color: black">Contributions of Learn with Train</p>
```
:::
:::
...
::: g-col-6
```{r}
lwt_people_served <- formatC(lwt_people_served, format = "d", big.mark = ",")
html_code <- paste0(
  '<div style="display: flex; flex-direction: column; align-items: center; height: 150px; border: 1px solid #042d33; border-radius: 5px; background-color: #042d33; color: white; position: relative;">',
  '<div style="position: absolute; top: 15%; width: 100%; display: flex; flex-direction: column; align-items: center;">',
  '<div style="display: flex; align-items: center; font-size: 2em; font-weight: bold;">', 
  ...
  '<span style="font-size: 1.25em; margin-left: 10px;">', bsicons::bs_icon("person-fill"), '</span></div>',
  '</div>',
  '<div style="position: absolute; bottom: 20px; width: 100%; text-align: center;">Youth Served </div>',
  '</div>'
)
htmltools::HTML(html_code)
```
